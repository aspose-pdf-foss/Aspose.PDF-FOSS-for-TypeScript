...
- **Optional content is honoured when rendering, not when extracting** — a hidden layer does not paint in `ToImage`/`ToSvg`/`ToHtml`, and that covers a marked-content section, an `/OC` on an image or form XObject, and an `/OC` on an annotation alike; `FlattenAnnotations` and `SearchAnnotations` follow the same rule, since both are defined by what a static render draws. But `GetText`, `GetTextFragments` and `GetPaths` still report a hidden layer's words and geometry, which is tracked. Rendering follows the document's **default** configuration (`/OCProperties /D`); applying a named configuration first is not yet expressible.
